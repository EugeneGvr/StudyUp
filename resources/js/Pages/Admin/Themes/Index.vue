<template>
  <div>
    ...
    <div class="mb-6 flex justify-between items-center">
      ...
      <inertia-link class="btn-indigo" :href="route('admin.themes.create')">
        <span>Create</span>
    ...
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr v-for="theme in themes.data" :key="theme.id" class="hover:bg-grey-lightest focus-within:bg-grey-lightest">
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center focus:text-indigo" :href="route('admin.themes.show', theme.id)">
              {{ theme.name }}
<!--              <icon v-if="organization.deleted_at" name="trash" class="flex-no-shrink w-3 h-3 fill-grey ml-2" />-->
            </inertia-link>
          </td>
          <td class="border-t w-px">
            <inertia-link class="px-4 flex items-center" :href="route('admin.themes.show', theme.id)" tabindex="-1">
